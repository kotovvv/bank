...
role:[
  ...
  computed: {
    adminComponent() {
      ...
      if (this.adminMenu == "operators") return operators;
      if (this.adminMenu == "report") return report;
    },
    userfio(){
      let a = this.user.fio.split(' ')
      if(a.length > 2) return this.role[this.user.role_id]+': ' + a[0]+' '+a[1][0]+'.'+a[2][0]+'.'
      return this.role[this.user.role_id]+': ' + a[0]
    }
  },
